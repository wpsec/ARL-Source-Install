...
- 访问日志已包含 `remote_user`、`request_time`、`upstream_status` 等字段，便于排查是否为 Basic Auth 401 挑战、上游异常或网络抖动导致重复认证。

### Q: Basic Auth 登录后能持续多久？

HTTP Basic Auth 本身没有 nginx 可配置的“登录有效期”参数。

- 是否持续复用凭证，主要由浏览器决定。
- 大多数浏览器会在同一浏览器会话内缓存同一 `realm` 的凭证。
- 关闭浏览器、清理站点数据、使用无痕模式、浏览器隐私策略、代理容器重启后再次访问，都可能重新触发认证弹窗。

如果希望“尽可能长”地减少重复输入，建议：

1. 保持使用同一浏览器会话，不要频繁清理站点数据。
2. 使用浏览器的密码保存/自动填充能力。
3. 结合访问日志排查是否存在异常 401 或容器频繁重建。

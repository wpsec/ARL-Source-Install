...
const ICP_TABLE_CELL_CLASS = 'min-w-0 max-w-full overflow-hidden align-top';
...
function ResultTable({ records }: { records: IcpRecord[] }) {
  ...
  return (
    <div className="overflow-x-auto">
      <table
        className="arl-fixed-table table table-zebra table-sm"
        style={{ minWidth: columns.reduce((total, [key]) => total + (ICP_RESULT_COLUMN_WIDTHS[key] || 180), 92) }}
      >
        <colgroup>
          {columns.map(([key]) => <col key={key} style={{ width: ICP_RESULT_COLUMN_WIDTHS[key] || 180 }} />)}
          <col style={{ width: 92 }} />
        </colgroup>
        <thead>
        ...
        <tbody>
          {records.map((record, index) => (
            <Fragment key={`${record.domain || record.service_name || 'record'}-${index}`}>
              ...
              {expandedIndex === index ? <tr key={`${record.domain || record.service_name || 'record'}-${index}-detail`}><td colSpan={columns.length + 1} className="min-w-0 max-w-full"><pre className="max-h-72 max-w-full overflow-auto whitespace-pre-wrap break-all rounded-box bg-base-200 p-3 text-xs leading-5">{JSON.stringify(record, null, 2)}</pre></td></tr> : null}
            </Fragment>
          ))}
        </tbody>
      </table>

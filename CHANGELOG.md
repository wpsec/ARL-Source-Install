...
- `[v3.1.44]` WIH 构建稳定性修复：修复 `wih` 规则模板 embed 路径（`lib/rules_embed.yaml`）导致的编译失败；Docker 构建阶段新增 Go 工具链离线优先逻辑，支持预置 `tools/go1.22.4.linux-amd64.tar.gz` 加速构建

...
- `[v3.3.1]` 端口扫描链路性能重构：`port_scan` 新增“分片执行 + 两阶段扫描（先快扫后精扫）”模式，先快速发现开放端口，再仅对高价值主机进行 `-sV/-O` 精扫；同时加入自适应降级策略（主机上限、单主机端口上限、超阈值自动关闭 `os_detect`）与阶段化日志，降低全端口/大目标任务对 worker 的长时间占用

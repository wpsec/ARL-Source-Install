...
## 2026-03-18（v3.3.1 ~ v3.3.14）

- `[v3.3.14]` 任务管理搜索增强：`任务名` 搜索框在保留“直接输入关键字搜索/回车搜索”现有模式的同时，新增已有任务名下拉建议能力；进入任务管理页后会自动拉取任务名候选，并在当前列表结果变化时同步补充建议项，支持“输入搜索 + 下拉选择”两种用法并复用现有同名任务查看与按任务名批量操作链路

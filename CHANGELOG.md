...
## 2026-09-16（未发布，v4.7.435）

- `全系统扫描信息悬浮展示统一改造`：`HoverCellValue` 改为基于实际横向/纵向布局溢出检测，接入 `ResizeObserver`、Portal 边界计算、键盘 Esc 和可选复制；`DataTable` 默认文本单元格及 TableModuleView、Dashboard、ICP 查询、AI 日志的扫描信息统一支持完整内容查看，移除字符长度与居中列排除规则，保留固定列宽和既有按钮/链接/截图交互。自动化测试、TypeScript 检查和生产构建已通过，真实浏览器布局与虚拟滚动验收待完成。

## 2026-09-08（未发布，v4.7.221）

- `统一 API 发布配置收口`：统一 API 开关现支持 YAML 与环境变量透传，并同步注入 web、worker 和 scheduler；生产默认保持关闭，异常回退默认开启，`shadow` 与 Rust stage 仅通过受控验收流程启用。
- `构建配置与快速开始文档补齐`：`.env.example` 明确必填项与可选项，补充 npm、PyPI、Playwright 国内镜像覆盖配置；README 快速开始补充凭据要求、直接启动与 Docker Compose 的差异，降低首次构建和启动时的配置误判。
- `发布验收与回归工具完善`：新增非敏感运行配置指纹和任务导出合并工具；附录 D/E 补齐失败即停、动态 worker 重启、任务范围采样、同 revision 双架构及 40/64 目标验收流程，并明确原始目标与任务证据不得入库。当前代码增量已完成，真实环境性能、生产 smoke、发布回滚和最终默认开关切换仍待验收。

## 2026-09-04（未发布）

- `真实环境诊断驱动修复`：x86 测试机 11 任务批次日志确认第四轮修复全面生效；据此修复 WIH 撞墙空产出仍记 covered（重投永久跳过）与 `host:` 空端口脏值生成非法 URL 两项实锤缺陷，并删除孤儿 `cloud_security_scan` 模块。详见 docs/03 第六批。
...
- `[v4.6.84]` `钉钉知识库分块写表列宽补齐`：针对计划任务推送钉钉知识库时，`系统服务` 等工作表在首个分块包含完整 5 列、后续分块又恰好尾列长期为空的场景，`write_sheet_values_to_workbook` 先按整张表生成了 `A201:E288` 这类 5 列范围，但 `update_workbook_range` 在二次归一化分块数据时又把尾部空列裁成了 4 列，最终被钉钉接口拒绝并报出 “The number of columns in the data does not match the number of columns in the range”。这一轮给 `update_workbook_range` 增加了 A1 range 列宽解析与强制补齐逻辑，分块请求现在会严格按目标范围宽度补空列，避免多 sheet 报告只在第二段或后续分块写入时出现 `partial_success`；对应回归测试也补齐了 `A201:E288` + 尾列空值的场景`
...
- `[v4.6.76]` `WIH去噪语义校正/URL资产去重` 收口：围绕 `WIH` 结果消费层继续把“看起来像风险”和“有证据证明是风险”拆开处理。`site / fileleak / url / vuln / nuclei_result` 的规则层与 `AI去噪` 默认提示词统一切到“证据优先”口径，新增对 `登录壳 / 统一认证入口 / 401/403/404 / 普通错误页 / 静态资源 / 参数校验失败 / 网络异常 / 仅规则命中无利用证据` 等弱信号的显式降权，只有在真实开放的 `接口文档 / 调试面 / 目录索引 / 敏感文件成功返回 / 凭据或业务成功证据` 出现时才允许提级。同时，`wih_url_probe` 不再向 `fileleak` 重复写入 URL 页面结果，`fileleak` 列表/导出/前端筛选默认排除 `wih_url_probe` 来源，目录扫描命中后也会反向清理同 URL 的 `url` 资产；周期任务复用上一轮 `wih_url_probe` 结果时也开始跳过当前任务里已存在于 `fileleak / url` 的同 URL，避免重复资产、重复导出和复用污染。`docs/plan/WIH提速与调度优化实施计划.md` 也同步补齐了第五阶段“AI 去噪可信度收口”的目标、规则和观测项，方便后续继续沿文档推进`

## 2026-04-11（v4.6.66）

- `[v4.6.75]` `AI去噪/WIH接口响应语义校正`：针对 `WIH接口`、`AI去噪` 之前过度依赖 `POST + import/export/admin` 等路径/方法特征，导致像“接口回复明确为未登录、权限不足、访问被拒绝、资源不存在、参数校验失败”这类场景仍被误判为 `高价值` 的问题，这一轮把 `回复报文` 正式接入 `WIH接口` 规则层和 AI 上下文。后端新增对 `ai_fill_response_packet / verification_response_packet / response_packet` 的统一解析，会抽取 `status_line / body_excerpt / JSON键 / 响应语义`，识别 `权限拒绝、鉴权失败、资源不存在、参数校验失败、业务成功、敏感字段返回` 等信号；规则结果在进入详情与落库前也会基于这些语义做最终校正，避免 `权限不足` 一类受保护接口继续被简单打成 `高价值`。同时，`默认AI去噪-WIH接口` 提示词明确要求优先使用 `回复报文` 做价值判断，`站点/目录扫描/URL信息/风险` 的默认 SOP 也一并收紧，减少继续仅靠路径关键字“脑补风险”的情况`
- `[v4.6.74]` `钉钉集成敏感配置真隐藏`：钉钉集成页原先会在 `/dingtalk_api/config/` 的读取/保存回包以及 `runtime_status` 中直接返回 `钉钉机器人 Token / Secret、CorpID、AppKey、AppSecret、操作者ID、工作空间ID、父节点ID`，前端即使后续做输入框遮罩，抓包仍然能直接看到真实值。这一轮将钉钉集成正式接入和 `API管理 / AI管理` 同级的敏感配置保护链：后端新增敏感字段配置状态计算、客户端脱敏返回、保存时未提交敏感字段自动回填、`/dingtalk_api/reveal/` 二次身份验证显示接口，以及运行状态中的敏感字段脱敏；前端新增“显示敏感配置/隐藏敏感配置”按钮、身份验证弹窗、配置占位提示与保存后自动回到隐藏态。这样默认加载、保存后的回显和普通抓包都不会再泄露这些钉钉关键配置，只有显式完成身份验证后才允许查看明文`
- `[v4.6.73]` `WIH默认参数/实施文档` 第二轮收口：在首轮 `WIH` 提速与调度优化落地后，继续把默认参数向“更激进但仍可控”的方向收紧。`ARL` 默认 `WIH_CONCURRENCY` 从 `6` 提升到 `8`，进一步提高站点级吞吐；周期任务轻量阶段和 `minimal` 回退阶段的进程级预算从 `900s` 明确收紧到 `120s`，并把配置注释改成“单批 `WIH` 子进程超时”口径，避免与 `WIH_LIGHT_RUNTIME_TIMEOUT_SEC` 这种 runtime 单页超时混淆。与此同时，独立 `tools/wih` 的默认 `--concurrency/-c` 也从 `2` 提升到 `4`，帮助单独运行 `./wih` 时也能获得更快的默认速度；`docs/plan/WIH提速与调度优化实施计划.md` 同步补齐了“当前实施状态、超时口径说明、建议默认值和下一轮精修方向”，方便后续持续调优和部署落地`
- `[v4.6.72]` `WIH提速/调度一致性` 分阶段优化落地：围绕 `docs/plan/WIH提速与调度优化实施计划.md` 的四阶段方案，`ARL` 侧完成了首轮端到端收口。周期任务下发时会透传 `task_schedule_id / task_schedule_name / task_schedule_run_number`，`web_info_hunter` 在执行前开始基于当前 `site` 指纹与同计划历史任务做保守复用，仅在 `site/title/status/http_server/body_length/favicon/finger` 完全一致时复用上一轮 `wih / wih_endpoint / wih_url_probe` 结果，减少周期任务重复深扫；`InfoHunter` 主扫描新增 `light -> full` 的保守 runtime 升级链，周期任务默认先走轻量 runtime，只有结果偏薄时才升级到完整预算，同时 `minimal` 回退改为真正轻量模式，避免超时后再次把深度 runtime 预算跑满；`urlfinder_sensitive` 阶段开始对候选 URL/HTML/JS 目标按接口/后台/参数/同 host 等风险信号排序，并新增“连续无新增批次”提前止损，减少大量低价值候选导致的长时间空转。与此同时，多 worker 下的长任务收尾一致性也同步收口：worker 启动恢复会先读取 Celery inspect 存活任务，避免把其它 Pod 正在执行的任务误标成 `error`；`task_schedule_run` 开始支持终态重查与状态校正，知识库写入与 `notify_on` 解耦，只要本轮 run 已结束就允许补写知识库，降低长任务被中途误判后“知识库永远不写”的概率`
- `[v4.6.71]` `WIH接口详情/复制交互` 轻提示优化：针对 `WIH接口详情` 里“复制请求URL/请求报文/回复报文”等按钮点击后反馈不明显，用户容易误以为按钮无效的问题，这一轮把主任务详情页里的成功提示统一收成右上角非阻塞浮动 toast。复制成功后会立即弹出 `xxx 已复制`，约 2 秒后自动消失，不再要求用户额外确认，也不会挡住继续点击其它按钮；这套交互同时覆盖 `TableModuleView` 里同类的复制、导出和轻操作成功提示，整体反馈更直接`
- `[v4.6.70]` `导出报告/WIH接口提取` 补齐 AI 相关列：针对任务报告 Excel 导出里 `WIH接口提取` 工作表只有基础接口字段，缺少 `AI填充参数 / 回复报文 / AI分析`，导致离线查看报告时无法直接判断接口是否已被 AI 填充、拿到了什么响应、价值分级又是什么的问题，这一轮为 `WIH接口提取` 表新增三列。`AI填充参数` 会按 `参数名 [位置/类型] = 值` 形式输出当前接口的 AI 补全结果；`回复报文` 会优先导出 `AI填充` 后拿到的响应包或摘要，在没有真实响应包时回退到验证响应或失败原因；`AI分析` 则直接复用扫描阶段已落库的 `AI去噪-WIH接口` 结果，导出为与前台一致的价值标签文本，便于报告阅读和后续人工复核`
- `[v4.6.69]` `WIH接口详情` AI填充测试失败展示修复：针对 `WIH接口详情` 中存在“AI填充参数已经生成，但填充后测试实际超时或失败时，顶部仍显示为 `已填充`、右侧回复报文又回退成旧的 `超过 WIH 接口轻量验证上限` 提示”的语义错位问题，这一轮补齐了 `AI填充` 的测试失败状态与前端展示优先级。后端在填充后低副作用测试抛出异常时会明确落成 `ai_fill_status=test_failed`，不再继续沿用笼统的 `filled`；前端对应新增 `测试失败` 状态映射，并让“回复报文”在没有真实响应包时优先展示本次 `AI填充` 测试的失败原因，例如 `ReadTimeout`，避免被更早阶段的轻量验证跳过说明覆盖，帮助工程师直接看出是“参数已填充，但测试请求没拿到响应”而不是“根本没测”`
- `[v4.6.68]` `WIH接口AI填充` 上限默认值与 AI 管理配置化：针对 `WIH接口AI填充` 在目标较多时频繁出现“超过上限，未继续处理”，但用户又无法在界面里直接调高预算的问题，这一轮将默认单次处理上限从 `60` 提升到 `200`，并把该预算正式接入 `AI管理 -> AI对话高级参数`。前端新增 `WIH接口AI填充上限` 数值输入框，保存后会和其它 AI 高级参数一起进入配置文件；后端则补齐了该字段在 `AI配置` 读取、回显和保存时与 `ARL.WIH_ENDPOINT_AI_FILL_MAX_TARGETS` 的映射，避免出现“界面可见但实际运行仍沿用旧默认值”的断层。这样默认体验更稳，同时也允许按现场算力、token 预算和任务规模灵活调节`
- `[v4.6.67]` `WIH接口AI填充` 连接异常回退修复：针对 `AI管理` 中 `DeepSeek` 连通性测试成功、`AI去噪-WIH接口` 也能正常调用，但 `WIH接口AI填充` 偶发在详情页直接落成 `AI 填充失败: ConnectionError`、且整条接口没有任何参数补全结果的问题，这一轮收口了 `wih_endpoint_ai_fill` 的异常处理链。AI 填充调用现在会在请求层内部捕获 `ConnectionError/ConnectTimeout/SSLError` 等传输异常，保留更完整的错误原因，并在短暂重试后自动回退到启发式参数补全，不再把异常直接冒泡到线程池外层导致整条记录被打成 `error`。同时，线程池兜底文案也改为输出“异常类型 + 详细原因”，避免前台只剩一个无法定位问题的 `ConnectionError` 类名；这样即使 AI 填充链路瞬时抖动，接口仍能继续生成启发式参数、执行低副作用验证，并把真正的错误上下文留在记录里供排查`
- `[v4.6.66]` 前台暗色主题与中文主按钮视觉收口：针对 `钛金黑 / 专业灰蓝 / 午夜科技 / 北欧极光` 四套主题在登录页和主操作区域出现的“按钮文字发飘、对比度不稳、中文全大写字距怪异”等问题，这一轮统一重调了暗色主题核心色板与主按钮规则。`钛金黑` 改为更接近 `VS Code Dark+` 的中性深灰基底与蓝色强调色，卡片、边框和氛围层整体更克制；`专业灰蓝 / 午夜科技 / 北欧极光` 也同步收紧了背景、卡片、边框、强调色和文字层次，减少原先偏灰发虚的观感。与此同时，所有 `bg-brand-accent` 实心主按钮开始统一走“深色主题高对比前景 + 渐变强调 + 阴影 + 中文友好字距”规则，登录页“登录系统”、修改密码弹窗“提交并重新登录”以及其它同类主按钮不再强制 `uppercase + tracking-wider`，避免中文按钮在暗色主题下出现奇怪的视觉断裂；主题色预览和品牌 Logo 材质色也同步对齐，整体暗色界面风格更稳定、更专业`

## 2026-04-10（v4.6.57）

- `[v4.6.65]` `WIH接口详情` 请求/回复报文双栏与 AI 填充联动：`WIH接口提取` 详情页最下方的报文区域改为左右双栏展示，左侧统一显示“请求报文”，在 `WIH接口AI填充` 已生效时优先展示 `AI` 填充后的请求报文，否则回退到 `WIH` 原始提取报文；右侧新增“回复报文”，会优先展示 `AI` 填充低副作用测试得到的响应报文，其次回退到 `wih_endpoint_probe` 轻量探测响应，再不行才显示摘要/说明。后端同时补齐了结构化响应预览落库，响应区域不再只有一句摘要，而是开始携带 `HTTP 状态行 + 关键响应头 + 截断后的响应体预览`，便于工程师在详情页里直接对照“填充后的请求”和“实际响应”做下一步验证`
- `[v4.6.64]` `WIH接口` AI填充与价值联动：新增 `WIH接口AI填充` 模块，进入 `web_info_hunter` 后会在 `wih_endpoint_probe` 之后对 `WIH` 提取出的结构化接口做第二轮“参数补全 + 低副作用验证”。该链路会优先复用原始 `request_packet/request_template` 中已有稳定值，只对 `<value>`、空值和明显无效值做类型化补全，并支持从 `application/x-www-form-urlencoded / json / query/path` 中恢复参数；`GET/POST/HEAD/OPTIONS` 会在安全范围内补一次轻量测试并沉淀 `响应摘要`，`DELETE/PUT/PATCH/TRACE/CONNECT` 以及 `multipart/二进制` 等高副作用请求则只给出提示不自动实测。`AI去噪-WIH接口` 现在会消费 `AI填充参数、AI填充说明、测试响应摘要` 继续判断 `高价值/中价值/无价值`，前端 `AI管理` 新增 `WIH接口AI填充` 开关与 SOP 管理项，`WIH接口详情` 也开始展示 `AI填充状态、参数建议、测试摘要和填充后报文`，便于后续自动化测试与人工复核衔接`
...
- `[v4.6.59]` `WIH主扫描` 大批次重跑与长尾耗时优化：针对域名任务进入 `wih_primary_scan` 后，单批站点数过大导致一个慢站点拖住整批 `WIH`，外层超时后又把已经完成的站点跟着整批重跑、最终把阶段拖到数小时的问题，这一轮对 `InfoHunter` 的 `WIH` 调度链做了两处收口。首先新增 `WIH_MAX_BATCH_SIZE`（默认 `12`），`wih` 批次大小不再按 `concurrency*6` 膨胀到几十个站点，而是按“最大批次上限 + 当前并发建议值”收口，减少单批 blast radius；其次当 `primary/minimal` 批次超时时，`ARL` 会先读取当前 `wih_result` 文件，保留已经落盘的部分结果，并从后续拆批/重试中剔除已完成站点，不再把这部分目标从头重复扫描。对应 `config.py/config.yaml.example` 与 `test_wih_timeout_split.py` 已同步补齐回归，重点覆盖“批大小收口”和“超时先收部分结果、只重试剩余站点”两条主链

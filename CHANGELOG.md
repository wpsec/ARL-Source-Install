...
## 2026-03-25（v4.1.24 ~ v4.2.16）

- `[v4.2.16]` 报告导出 AI 去噪字段补齐：`Excel/HTML` 报告中的 `站点 / 目录扫描 / SSL证书 / URL信息 / 风险 / PoC风险` 工作表统一新增 `AI分析` 列，导出内容仅读取扫描阶段已落库的 `ai_denoise_result` 结果（不触发实时 AI 调用）；其中 `风险` 表对 `npoc` 与 `nuclei` 来源分别回填对应模块 AI 结论，批量导出的 `站点` 合并视图按“危险优先、AI来源优先”选择更高价值 AI 分析结果展示

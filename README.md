...
- 策略配置（新建/编辑）统一固定 `domain_config.domain_brute=true`，不再暴露域名爆破开关语义
- 策略表单文案调整为“默认字典模式”，并提示仅在任务未指定域名爆破字典时生效

### v3.0.19（2026-03-11）

- 新建任务中隐藏“域名爆破”勾选开关，避免与字典配置造成重复认知
- 任务提交时固定 `domain_brute=true`，保持原有扫描行为不变

### v3.0.18（2026-03-11）

- 任务管理异常可视化：任务状态为“异常”时支持点击查看详情
- 新增异常详情弹窗：展示异常时间、异常阶段、异常说明与 traceback 日志
- 后端新增任务异常落库：统一记录 `last_error` 与 `error_logs`，便于问题定位

### v3.0.17（2026-03-11）

- 新建任务端口扫描范围支持 `custom` 自定义
- 选择 `custom` 后显示自定义端口输入，并在提交前校验格式
- `/task/` 接口新增 `port_custom` 入参与后端校验，非法值会直接提示

### v3.0.16（2026-03-11）

- 新建任务文案优化：统一命名为“域名爆破字典”，移除“任务级”表述
- 交互优化：选择域名爆破字典后，默认字典模式自动标记为不生效，避免误解

### v3.0.15（2026-03-11）

- 新建任务新增“域名爆破字典”可选覆盖项：按任务选择字典，不选则走配置管理默认字典
- 任务后端增强：`/task/` 接口支持 `domain_dict` 入参并校验文件存在
- 域名任务执行增强：优先使用已选域名爆破字典，未配置时自动回退 `domain_brute_type` 与系统默认

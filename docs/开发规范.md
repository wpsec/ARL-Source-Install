...
本文档定义了ARL-Source-Install项目的开发规范，包括代码风格、提交规范、注释规范、测试规范等。
...
- `README.md` 的“更新日志”仅记录大版本摘要（MAJOR/MINOR 级别），用于快速了解阶段性能力变化
- `CHANGELOG.md` 负责维护详细版本变更（包含 PATCH 级别），每条变更需标注对应版本号
- 新增功能或修复时，必须同步更新 `CHANGELOG.md`；如涉及阶段性能力演进，再同步更新 `README.md` 摘要
...
- CI/CD 流水线包含：
  ...
  - 安全扫描
  - 性能测试
- 本仓库要求启用 `core.hooksPath=.githooks`，提交前自动更新 `ARL/version.txt`
...
- 维护更新日志（`README.md` 记录大版本摘要，`CHANGELOG.md` 记录详细版本）
- 每次提交必须更新 `ARL/version.txt`（由 `.githooks/pre-commit` 自动执行 patch +1）

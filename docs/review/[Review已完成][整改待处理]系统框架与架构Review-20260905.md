...
| 发布前门禁 §9 全部动态项（40/64、双架构、SSRF/越权动态测试） | **未完成——环境/授权依赖**：执行手册固化为 `docs/history/[已归档]06-附录E-计划6发布验收runbook-双架构与40-64目标.md`；安全动态测试归用户最终阶段安排 | 附录 E §6 完成定义 |
...
`docs/history/[已归档]紧急修复-统一发现系统数据与状态边界收口.md` §九 T5 复判定案：

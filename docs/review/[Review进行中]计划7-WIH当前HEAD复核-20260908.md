...
- 复核基线：当前分支 `newUI`，HEAD `9da5ab9a`（计划 5/6/7 离线闭环、EvidenceGraph/主题门禁、监控摘要图标与顶部操作栏布局、登录页窄屏修复、服务信息重挂载回归、IP 阶段轻依赖测试、侧边栏布局收口和总计划索引均已提交）；工作区另有并行的监控摘要布局微调，未纳入本次文档提交。
...
上述内容分别由 `docs/plan/[未完成]06-附录E-计划6发布验收runbook-双架构与40-64目标.md`、`docs/plan/[未完成]07-计划7-WIH智能API发现与受控验证可行性分析.md` 和暂停中的计划 5 runbook 管理。

...
- 整改状态：原始范围（第 4-6 批发现项）完成；**第 8 批后独立复审理新开启后续轮**——2 P0 + 2 P1 由紧急修复专项承载（§9 复审记录），原 P1-05（parent_target 脱敏）经用户裁定撤销并触发三层数据契约改判（附录A §4.16）。轮 1：P0-01、P1-10（其 URL 清洗语义后被 §4.16 部分改判，历史条目保留）；轮 2：P0-02~P0-04、P1-06~P1-09、P1-11、P2-13、P2-14；第 8 批：P0-05 事件接入与 P1-12 实施（`wsdl_operation_total` 落地）。原始范围仅余 P2-15（非阻断）。
...
第 8 批提交后的独立复跑复审给出 **Request changes**：范围边界与异常恢复问题测试绿不能覆盖。确认 2 项 P0（浏览器 GraphQL 绕过范围校验；页面/JS 回流失效可触发外域文档 fetch）、2 项 P1（浏览器 REST 请求形态未入幂等签名；Endpoint queued 无恢复）与 1 项原列 P1（parent_target 脱敏）。执行版文档明确：**旧 Review 保留为历史证据，不静默改写**；上述修复不在本文件展开，统一由 `docs/plan/[进行中]紧急修复-统一发现系统数据与状态边界收口.md`（T0-T6）承载与验收。
...
对本文件的净影响：文档标题 `[整改已完成]` 就其原始审查范围（第 4-6 批发现项 P0-01~P2-15）仍然成立；第 8 批消费面新增问题的状态以紧急修复文档 §六/§八 为准，不得据本文件 Accept 复判宣称第 9 批放行。

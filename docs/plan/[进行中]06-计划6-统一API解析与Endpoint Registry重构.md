...
状态：第 1-7 批解析器面已实施（2026-09-05，见文末实施进度；第 6 批为"GraphQL 文档 Parser 完成，运行时事件接入未完成"——2026-09-06 用户决策 P0-05，事件接入随第 8 批完成）；第 4-6 批 Review 整改轮 2 代码已实施（2026-09-06：T1/T2/T3 + P1-08/P1-09 + P2-13）；**第 8 批 Endpoint Registry 消费方接入已实施（2026-09-06，T8-1~T8-6：P1-12 幂等键纳入 api_type、监控入口切换、WIH endpoint probe/URL Probe 消费统一候选、浏览器 GraphQL 事件 operation 级拆解、JS/页面 GraphQL 记录面回流、P0-05 三来源合并门禁）。**第 8 批经独立复审发现 2 P0（范围越界）+2 P1（REST 形态签名、queued 恢复）+1 项设计改判（公开资产观测值不得脱敏，原 P1-05 撤销），第 9 批暂停，先执行 [紧急修复：统一发现系统数据与状态边界收口](<紧急修复-统一发现系统数据与状态边界收口.md>)（T0 契约冻结→T1-T4→T5 边界复判→T6 集成复核）；T1-T6 提交后经 R6 提交复核（1 P0 桥接绕过范围闸 + 2 P1 lease fencing/形态摘要碰撞 + 2 P2 兼容项）再次纠偏，**修复于 2026-09-06 全部完成（九件合跑 257 项，见紧急修复文档 §九 R6 节）；第 9 批阶段调度与 WAF 隔离已实施（2026-09-06，`d8ce750a`，合跑 261 项）；第 10 批 Rust 纯数据层已实施（2026-09-06：is_api_doc_candidate 三面同口径、unified_* 批量函数 + `RUST_ACCEL_API_UNIFIED_MODE` 默认 shadow、backflow 批量接线、统一面 golden corpus aarch64/py3.10.20 `--run-native --strict-order` 全绿、CPU 闸 normalize 15.4x/method 过闸 hint/dedupe 不过闸；十三件合跑 291 项）；第 11 批本地闭环部分已完成（2026-09-06：F4/F6 余留项+判别 golden case、compose 等价全栈 arm64 容器回归 848 项与 hygiene 149/149 clean，见第 11 批首批本地闭环项节与计划 3；**当前 HEAD 复核 T11-0 修复已实施（2026-09-07：rust stage 级硬门禁/网络等待真实口径/finally 收口 flush/lease 配置接线/degraded_reason 资产面 + 附录E runbook 落盘，附录A §4.20 契约登记）**；amd64 qemu 构建经用户决策暂不跑，余留 40/64 目标、真机基准与模式升级决策），默认切换仍禁止**。契约冻结面见 [06-附录A](<../completed/[已完成]06-附录A-API契约冻结清单.md>)（URL 观测语义以 §4.16 三层数据契约为准）。
...
- 第 9 批放行条件见紧急修复文档 §八与 §九 R6 节：R6 复核曾将"可恢复开工"修正为"修复后恢复"，2026-09-06 全部修复完成（桥接三态收口、claim fencing、shape overflow digest、位置参数兼容、注释/状态口径统一），第 9 批代码专项解除暂停；容器/40-64 验收补齐前 `API_UNIFIED_ENABLE` 默认切换保持禁止。

### 第 9 批：阶段调度与 WAF 隔离（2026-09-06 完成，提交 `d8ce750a`）
...
未完成项（如实登记）：provider 级细分耗时（Nuclei/AfroG 等属计划 3 风险阶段面）；WAF"不发第二请求"端到端度量随第 11 批容器验收；P2-15 parser 分文件仍未拆（本批 parser 文件零改动，继续挂起评估）。

### 第 10 批：Rust 纯数据层和性能门禁（2026-09-06 实施）
...
| 双架构基准 | qemu x86_64 仿真下四项 ratio 全部过闸（normalize 0.050/hint 0.297/method 0.497/dedupe 0.656）——**该数据不可采信**：仿真对 CPython 的减速远大于对 native 码，ratio 系统性乐观 | 本轮 amd64 证据仅确认"编译+16 单测+golden 跨架构一致+基准可执行"；**CPU 闸判定只认 x86 真机（第 11 批发布验收 runbook（附录 E）窗口）复跑 `bench_api_unified_rust.py` 的数据**，aarch64 第二轮（native 环境）过闸结论维持为当前唯一有效门禁证据 |
...
### 第 11 批首批本地闭环项（F4/F6，2026-09-06）

| 项 | 处置 | 证据 |
|---|---|---|
| F4 legacy fallback 动态键未静态声明 | `rust_accel._STATS` 补 `last_html_fallback_reason`/`last_js_endpoint_fallback_reason`（`_fallback()` 对 html/js_endpoint stage 写动态键但静态声明缺位，观测口径盘点不完整） | 三件合跑 OK；`_STATS` 键面与 `_fallback` 全部 stage（extract/html/js_endpoint/rank + unified 四通道）逐一对账 |
| F6 legacy WIH 提取面 trim 语义差 | lib.rs legacy 通道凡镜像 Python `.strip()` 的判定点统一改 `py_strip()`（clean_candidate、extract_host、strip_route_method_suffix、has_route_template_markers、is_noise_single_segment_path、is_js/is_non_js_resource_path、strip_url_annotation、is_js_url、normalize_url host、append_page_record 空判、scope domains、pattern_tokens、html/js core 输入集与页空判、form method/field name、rank record_type）；调用点与 Python 基线逐点核对（url_candidate_filter/urlfinder_extract/web_info_intel_utils/urlfinder_sensitive_scan/page_intel_scan），非 strip 对应点未改（`safe_site`/`is_blocked_url` 两侧均无 trim，保持不动） | cargo test --locked 21 项全绿（新增 5 项 legacy 行为钉：clean_candidate/host+js_url/route-suffix、pattern_tokens、纯控制符记录丢弃、rank record_type）；legacy golden corpus 新增判别 case `rank-control-char-record-type`（python 字段=生产容器内 `UrlfinderSensitiveScanner` 真实方法实测，非手推） |
...
统一面 corpus 4 case 与 `api-unified-golden.py --check` 在新 lib.rs 下零漂移（`arl-py310` 容器复验 ok=True）；`compileall`、`git diff --check`、hygiene（两改动测试文件 clean）通过。第 11 批余留：40/64 目标、`RUST_ACCEL_API_UNIFIED_MODE` 升级决策、端到端 ≤5%、amd64 真机基准（附录 E 发布验收 runbook；qemu 构建经用户决策暂不跑，`bash scripts/run-container-regression.sh linux/amd64 amd64` 可重放）。**compose 全栈容器回归（arm64）已完成**（2026-09-06 第二轮，计划 3 登记：Ran 848、74 例 Mongo 环境项清零、hygiene 容器 149/149 clean、发布链镜像内双 corpus `--run-native --strict-order` 全绿含 F6 判别 case；剩余 FAIL/ERROR 归网络依赖与既有合跑交错两类存量）。

### 第 11 批 T11-0：当前 HEAD 复核的契约/门禁修复（2026-09-07 实施；来源 `docs/review/[Review进行中]计划6第9-11批当前HEAD复核-20260907.md`）

| 项 | 交付 | 证据 |
|---|---|---|
| P1-01 rust 全局模式误启用未过闸 stage | `RUST_ACCEL_API_UNIFIED_RUST_STAGES`（逗号分隔 stats_prefix；类默认 `unified_normalize,unified_method`=已过闸+native 环境实测；四处接线同 mode 键），`_unified_batch` 白名单外 stage 在 rust 下**收敛 shadow**（双跑观察保留、基线输出、`used_native=False`），每 stage 首拒 warning；metrics 增 `requested_mode`/`stage_gate` 证据位 | `TestUnifiedStageGate` 5 项（默认拒 hint/dedupe+mismatch 计数在位、allowlist 采纳、空表全拒、未知 token 忽略；含 `python3 -c import app.scheduler` 型独立入口冒烟）；既有采纳类用例显式放行改造；api 九件合跑 309 项全绿 |
| P1-02 network_wait 计时范围 | `fetch_text` 新增可选出参 `timing_out`，成功/失败两路径写真实 `utils.http_req` 挂钟、缓存/singleflight/DNS/调度等待恒不写；Queue `network_wait_sec` 只累计该值 | `TestFlushTextTimingOut` 3 项（真实请求写值/拒连仍写/缓存命中零网络）+ Queue `_default_fetch` 累计测试 |
| P1-03 lease 键无标准接线 | `API_ENDPOINT_CLAIM_LEASE_SEC` 入 `UNIFIED_API_CONFIG_DEFAULTS`(900) + Config 类默认 + `_ARL_POSITIVE_INT_KEYS`（yaml+env 双环）+ runtime yaml 正值表；`config_lease_sec` 补非正数/非法回退常量 | `ConfigLeaseWiringTest` 5 项（默认表/上下文优先/非法回退/Config 属性/claim deadline=now+30 实测）；`test_sync_runtime_config` 绿 |
| P2-01 degraded 原因不落资产面 | `UnifiedApiEndpoint.degraded_reason` 尾位增量字段 + `ENDPOINT_DEGRADED_REASONS` 受控枚举 + `sanitize_endpoint_degraded_reason`（64 截断→白名单→other；不携带响应/URL 值）；`mark_endpoint/probe_report` 透传，非 degraded 迁移清除旧原因；wih_orchestrator host_waf_blocked 路径传 `host_waf_blocked` | `EndpointDegradedReasonTest` 3 项（快照 blob 含原因/枚举外收敛 other/requeue 清残留）+ orchestrator 测试加 `reported_reasons` 透传钉 + models 契约 3 项（to_dict 尾键/枚举卫生/位置参数不漂移，R6-P2-01 同型） |
...
- amd64 功能面（同机 compose 等价全栈，镜像 `arl-regression:amd64`）：**Ran 875（与 arm64 计数持平），failures=31，errors=123，skipped=18**；hygiene 148 clean + `test.test_wih` 裁定完成：隔离直跑 398.8s OK（真网 WIH 集成非污染），工具子进程路径 timeout 且 kill 失效→工具健壮性调查项；native smoke 通过；双 corpus `--run-native --strict-order` exit=0/0。
...
- [已完成] 第 4-6 批 Review 余留项处置（2026-09-06 第 10 批）：Rust 原生面关键字同口径扩展（graphql/graphiql/wsdl，三面钉 + legacy 窄集合钉）闭环；P2-15 解析器分文件重构**评估结论=缓拆**（触发条件登记见第 10 批实施节 T10-F）。
- [已完成] 第 9 批阶段调度与 WAF 隔离（2026-09-06，`d8ce750a`）：api_doc/endpoint_probe 独立流量类别与并发额度、文档与探测互不连坐、主机级封禁 degraded/host_waf_blocked 收口、api_stage_wall/cpu/network_wait 计时与 pending/waf-blocked/外部边界观测；九件合跑 261 项全绿。
- [已完成] 第 10 批 Rust 纯数据层（2026-09-06）：批量归一化/method/type hint/去重合并四函数 + 三模式 adapter（默认 shadow）+ backflow 批量接线 + 统一面 golden corpus（aarch64 Linux/py3.10.20 `--run-native --strict-order` 全绿）+ CPU 基准（normalize 15.4x/method 过闸，hint/dedupe 不过闸不升级）；`API_UNIFIED_ENABLE` 默认 False、rust 模式未默认启用。
- [已完成] 第 11 批 T11-0（复核裁定 Accept，见复核文档 §10）（2026-09-07，当前 HEAD 复核 P1-01/02/03 + P2-01/03 收口 + 附录E runbook 落盘）：
  rust stage 级硬门禁（白名单外收敛 shadow + `stage_gate` 证据位）、`api_stage_network_wait_time` 只计真实 `http_req`、阶段 metrics finally 收口 + 水位增量 flush、`API_ENDPOINT_CLAIM_LEASE_SEC` 四处标准接线、`degraded_reason` 受控枚举进资产面（附录A §4.20）；api 九件合跑 309 项全绿。
...
- [未完成] 全量回归与发布验收（第 11 批）：40/64 目标、双架构基准复跑（amd64 真机，附录 E runbook）、`RUST_ACCEL_API_UNIFIED_MODE` 升级决策、端到端 ≤5% 验证。**首批本地闭环项已完成（2026-09-06）**：F4 fallback 静态键补齐、F6 legacy 提取面 py_strip 对齐 + 判别 corpus case（旧 .so 实锤 missing、新 .so 五 case 全绿）；`test_web_info_intel` 容器执行已随 arm64 首轮闭环（计划 3 登记）。
...
当前判定（2026-09-07 复核 §10 分状态：T11-0/T11-1 整改 Accept；第 11 批发布验收进行中——整体仍不能关闭）：计划 6 第 1–7 批解析器面 [已完成]（OpenAPI/Postman/GraphQL/WSDL 文档解析全部接管，golden 无漂移）；第 4-6 批 Review 整改轮 1（P0-01/P1-10）与轮 2（P0-02/P0-03/P0-04 + P1-06~P1-09/P1-11 + P2-13）[已完成]；**第 8 批 Endpoint Registry 消费方接入 [已完成]（T8-1~T8-6：P1-12/P0-05 闭环，probe/URL Probe/监控入口消费统一层，三来源合并与泄露门禁测试过，九件合跑 248 项全绿，R6 提交复核纠偏后 261 项）**；第 9 批（阶段调度与 WAF 隔离）[已完成]（`d8ce750a`，类别隔离+互不连坐+阶段计时）；第 10 批（Rust 纯数据层）[已完成]（四批量函数+shadow 默认+golden 严格一致+CPU 闸判定：normalize/method 过闸、hint/dedupe 不过闸不升级；`RUST_ACCEL_API_UNIFIED_MODE` 默认 shadow，rust 模式启用挂第 11 批）；第 11 批（全量回归与发布验收）[未完成]。`API_UNIFIED_ENABLE` 默认 False，生产默认行为未切换——"代码具备入口"与"默认生效"分别记账；**切换默认的全部代码前置门禁（轮 1/轮 2 P0 项 + P0-05/P1-12）已闭环，剩余是发布流程决策（§十三 shadow→双写→小范围→全量）而非代码阻断项**。第 2/3 批 Review 的终态与候选 drain 前置已在 2026-09-05 终态修复轮闭环，第 3–9 批未新增绕过统一收尾的消费通道（残余候选保持开放态，finalizer 显影语义不变）。

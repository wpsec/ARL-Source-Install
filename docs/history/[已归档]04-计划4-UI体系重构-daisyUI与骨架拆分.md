...
- [已完成] 离线构建链：`tsc --noEmit` 通过，Vitest `19` 个测试文件、`118` 项测试通过，Vite production build 通过，dist 入口资产和 SHA-256 清单自洽。

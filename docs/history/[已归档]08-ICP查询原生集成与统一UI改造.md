...
> 当前状态：[已归档]。代码、配置和本地自动化测试已完成；真实 ICP 接口验证码、上游限流和目标环境容器运行由后续 review/test 在授权环境验证，本文件保留验收入口。
...
本计划定义的代码和本地验收范围已完成；真实接口和发布环境验收属于部署方操作，入口和脱敏要求保留在第 10.2 节。
...
7. [ ] 由用户在授权测试环境进行真实 ICP 查询验证，根据真实返回结构补充字段兼容和异常处理。
...
- 新增完整协议 fixture，覆盖令牌获取、验证码图片/校验、查询请求的顺序执行、凭证传递和结果归一化；当前 ICP 定向回归为 49 项通过、1 项因本机未安装 Pillow/numpy 跳过。
...
- 2026-09-09 测试服务器首次真实单次查询：任务创建、arlweb 投递、Worker 执行、失败状态回写和前端展示均正常；工信部上游返回风控拦截，当前未通过真实数据验收。该结果说明需要在测试环境更换可用出口或配置可访问官方接口的代理后重试，不能据此宣称真实数据验收通过。
...
6. 本文件已按代码完成归档；真实接口、验证码、限流和 AMD64 容器运行通过后，直接在本归档文档补充验收记录，并将第 10 节第 7 项改为 `[x]`。

开发代码和本地测试已完成，文档已移动至 `docs/history/`；真实接口、容器运行和发布验收仍由后续 review/test 闭环。

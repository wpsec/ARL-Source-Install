...
状态：**开发完成；x86 放行未执行**。开发期离线代码门禁、规则对账命令、容器加载检查、unified 灰度和回滚步骤已固化；真实 Mongo/Redis/RabbitMQ、x86 容器和观测期按用户安排暂不执行。
...
- 放行通过后，把门禁 1 的 `/tmp/fpdiff.out` 与门禁 3 采样片段归档进 `docs/history/[已归档]03-计划3-实施批次与验收回归.md` 对应批次，作为

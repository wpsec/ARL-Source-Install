...
func Options() *datatype.Option {
	...
	flagset.CreateGroup("runtime", "运行参数",
		...
		flagset.IntVarP(&option.RuntimeTimeoutSec, "runtime-timeout", "", 60, "运行时采集超时(秒)"),
		flagset.IntVarP(&option.RuntimeMaxPages, "runtime-max-pages", "", 12, "运行时探索最大页面数"),
		flagset.IntVarP(&option.RuntimeMaxActions, "runtime-max-actions", "", 32, "运行时探索最大交互动作数"),
		flagset.IntVarP(&option.RuntimeMaxRequests, "runtime-max-requests", "", 180, "运行时采集最大请求数"),

...
type Option struct {
	...
	RuntimeEnable      bool   // --runtime-enable：启用运行时参数采集
	RuntimeDriver      string // --runtime-driver：运行时采集驱动(noop/external/playwright)
	RuntimeCommand     string // --runtime-command：运行时采集命令或 playwright 覆盖命令
	RuntimeTimeoutSec  int    // --runtime-timeout：运行时采集超时（秒）
